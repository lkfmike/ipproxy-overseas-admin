...
const fetchUserList = async (query: string) => {
  ...
  try {
    const res = await request.get('/api/web/account/list', {
      params: {
        page: 1,
        size: 50,
        email: query
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/web/gateway/list', {
      params: {
...
const handleToggleAutoRenew = async (row: any, val: boolean) => {
  ...
  try {
    const res = await request.post('/api/web/gateway/update', { id: row.id, autoRenew: val })
    const data = res as any
    if (data.code !== 200) {
      row.autoRenew = !val
    ...
    if (valid) {
      ...
      try {
        const url = dialogType.value === 'create' 
          ? '/api/web/gateway/create' 
          : '/api/web/gateway/update'
...
const handleDelete = async (row: any) => {
  try {
    const res = await request.post('/api/web/gateway/delete', { id: row.id })
    const data = res as any
    if (data.code === 200) {
...
const handleBatchRenew = async () => {
  ...
  try {
    ...
    try {
      const res = await request.post('/api/web/gateway/batch-renew', { ids, uid })
      const data = res as any
      if (data.code !== 200) {
